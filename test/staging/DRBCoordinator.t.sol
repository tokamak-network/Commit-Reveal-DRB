...
contract DRBCoordinatorTest is BaseTest {
    ...
    function deactivate(address operator) public {
        address[] memory activatedOperatorsBefore = s_drbCoordinator
            .getActivatedOperators();
        vm.startPrank(operator);
        s_drbCoordinator.deactivate();
        vm.stopPrank();
        address[] memory activatedOperatorsAfter = s_drbCoordinator
            .getActivatedOperators();
        assertEq(
            s_drbCoordinator.getActivatedOperatorIndex(operator),
            0,
    ...
    function test_RequestRandomNumber() public make5Activate {
        ...
        ) = s_consumerExample.getRequestStatus(requestId);
        ...
        address[] memory activatedOperatorsAtRound = s_drbCoordinator
